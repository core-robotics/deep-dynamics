...
def test_hyperparams(model_cfg, log_wandb):
    ...
    for dir in os.listdir("../output/{}".format(model_name)):
        ...
        with open(
            os.path.join("../output/{}".format(model_name), dir, "scaler.pkl"), "rb"
        ) as f:
            scaler = pickle.load(f)
        test_dataset = string_to_dataset[param_dict["MODEL"]["NAME"]](
        ...
        try:
            model_file = sorted(
                os.listdir("../output/{}/{}".format(model_name, dir)), key=numbers
            )[-1]
            model.load_state_dict(
                torch.load(
                    os.path.join("../output/{}/{}".format(model_name, dir), model_file)
                )
            )
        except:
            continue
        ...
        if log_wandb:
            api = wandb.Api()
            run = api.runs(
                path="cavalier-autonomous/{}".format(model_name),
                filters={"display_name": dir},
            )[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Log test values to wandb experiment",
    )
