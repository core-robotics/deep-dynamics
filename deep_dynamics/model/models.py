...
import torch
from deep_dynamics.model.build_network import (
    build_network,
    string_to_torch,
    create_module,
)
...
class ModelBase(nn.Module):
    def __init__(self, param_dict, output_module, eval=False):
        ...
        if eval:
            self.loss_function = string_to_torch[
                self.param_dict["MODEL"]["OPTIMIZATION"]["LOSS"]
            ](reduction="none")
        else:
            self.loss_function = string_to_torch[
                self.param_dict["MODEL"]["OPTIMIZATION"]["LOSS"]
            ]()
        self.optimizer = string_to_torch[
            self.param_dict["MODEL"]["OPTIMIZATION"]["OPTIMIZER"]
        ](self.parameters(), lr=self.param_dict["MODEL"]["OPTIMIZATION"]["LR"])
        ...
        self.sys_params = list(
            [*(list(p.keys())[0] for p in self.param_dict["PARAMETERS"])]
        )
    ...
    def test_sys_params(self, x, Ts=0.02):
        _, sys_param_dict = self.unpack_sys_params(
            torch.zeros((1, len(self.sys_params)))
        )
        ...
        alphaf = (
            steering
            - torch.atan2(
                self.vehicle_specs["lf"] * state_action_dict["YAW_RATE"]
                + state_action_dict["VY"],
        ...
        alphar = (
            torch.atan2(
                (
                    self.vehicle_specs["lr"] * state_action_dict["YAW_RATE"]
                    - state_action_dict["VY"]
                ),
                torch.abs(state_action_dict["VX"]),
            )
        ...
        Frx = (
            (sys_param_dict["Cm1"] - sys_param_dict["Cm2"] * state_action_dict["VX"])
            * throttle
            - sys_param_dict["Cr0"]
        ...
        dxdt[:, 0] = (
            1 / self.vehicle_specs["mass"] * (Frx - Ffy * torch.sin(steering))
    ...
    def init_hidden(self, batch_size):
        ...
        hidden = (
            weight.new(self.rnn_n_layers, batch_size, self.rnn_hiden_dim)
            .zero_()
            .to(device)
        )
        return hidden
    ...
    def __init__(self, param_dict, eval=False):
        ...
        class GuardLayer(nn.Module):
            def __init__(self, param_dict):
                ...
                self.coefficient_ranges = torch.zeros(len(param_dict["PARAMETERS"])).to(
                    device
                )
                self.coefficient_mins = torch.zeros(len(param_dict["PARAMETERS"])).to(
                    device
                )
            ...
            def forward(self, x):
                ...
                guard_output = (
                    self.guard_activation(guard_output) * self.coefficient_ranges
...
class DeepPacejkaModel(ModelBase):
    ...
    def differential_equation(self, x, output, Ts=0.02):
        ...
        Ffy = sys_param_dict["Df"] * torch.sin(
            sys_param_dict["Cf"]
            * torch.atan(
                sys_param_dict["Bf"] * alphaf
                - sys_param_dict["Ef"]
        ...
        Fry = sys_param_dict["Dr"] * torch.sin(
            sys_param_dict["Cr"]
            * torch.atan(
                sys_param_dict["Br"] * alphar
                - sys_param_dict["Er"]
        ...
        dxdt[:, 0] = (
            1
            / self.vehicle_specs["mass"]
            * (sys_param_dict["Frx"] - Ffy * torch.sin(steering))
            + state_action_dict["VY"] * state_action_dict["YAW_RATE"]
        )
        dxdt[:, 1] = (
            1 / self.vehicle_specs["mass"] * (Fry + Ffy * torch.cos(steering))
...
class DeepDynamicsModelF1T(ModelBase):
    def __init__(self, param_dict, eval=False):
...
class SlipF1TModel(ModelBase):
    ...
    def differential_equation(self, x, output, Ts=0.02):
        ...
        alphaf = steering - torch.atan2(
            self.vehicle_specs["lf"] * state_action_dict["YAW_RATE"] + vy, torch.abs(vx)
        )
        alphar = torch.atan2(
            (self.vehicle_specs["lr"] * state_action_dict["YAW_RATE"] - vy),
            torch.abs(vx),
        )
        Ffy = sys_param_dict["Df"] * torch.sin(
            sys_param_dict["Cf"] * torch.atan(sys_param_dict["Bf"] * alphaf)
...
class DeepDynamicsModelCN7(ModelBase):
    ...
    def differential_equation(self, x, output, Ts=0.01):
        sys_param_dict, _ = self.unpack_sys_params(output)
        state_action_dict = self.unpack_state_actions(x)
        steering = state_action_dict["STEERING_FB"] + state_action_dict["STEERING_CMD"]
        throttle = state_action_dict["THROTTLE_FB"] + state_action_dict["THROTTLE_CMD"]
        ...
        slip_angle = torch.atan2(state_action_dict["VY"], state_action_dict["VX"])
        Frx = self.vehicle_specs["mass"] * throttle * torch.cos(slip_angle)
        Ffy = sys_param_dict["D"] * torch.sin(
            sys_param_dict["C"] * torch.atan(sys_param_dict["B"] * alphaf)
        )
        Fry = sys_param_dict["D"] * torch.sin(
            sys_param_dict["C"] * torch.atan(sys_param_dict["B"] * alphar)
        )
        dxdt = torch.zeros(len(x), 3).to(device)
        dxdt[:, 0] = (
            1 / self.vehicle_specs["mass"] * (Frx - Ffy * torch.sin(steering))
...
string_to_model = {
    ...
    "DeepDynamicsF1T": DeepDynamicsModelF1T,
    "SlipF1T": SlipF1TModel,
    "DeepDynamicsCN7": DeepDynamicsModelCN7,
}
...
string_to_dataset = {
    ...
    "DeepDynamicsF1T": DeepDynamicsDataset,
    "SlipF1T": DeepDynamicsDataset,
    "DeepDynamicsCN7": DeepDynamicsDataset,
}
