...
def write_dataset(csv_path, horizon, save=True):
    with open(csv_path) as f:
        ...
        for row in csv_reader:
            ...
            beta= float(row[column_idxs["beta"]])
            ...
            odometry.append(np.array([v, beta, vtheta, throttle, steering]))
            poses.append([float(row[column_idxs["px"]]), float(row[column_idxs["py"]]), float(row[column_idxs["yaw"]]), v, beta, vtheta, throttle, steering])
